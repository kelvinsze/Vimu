...
| **App Name** | `Mivu` | 应用名称 |
| **Bundle Identifier** | `com.kelvinsze.mivu`（或你自定义的 ID） | 必须与 Xcode 工程配置一致 |
...
> Mivu 是一款轻量级的局域网视频播放器和媒体接收器，专门用于播放用户自行选择的媒体内容。它支持 HTTP/HTTPS 视频播放、HLS（.m3u8）流媒体、符合行业标准的局域网媒体投送（UPnP/DLNA）以及个人媒体服务串流。
>
> 在支持 Apple "Video in Car"（车载视频）的 CarPlay 系统上，Mivu 允许用户在车辆系统明确指示视频播放处于安全可用状态（例如车辆挂 P 档/停车状态）时播放选定的视频内容。
>
> **安全与系统合规性：**
> 1. Mivu 完全依赖 Apple CarPlay 原生框架与车机系统状态（`CPSessionConfiguration` 的视频可用性属性）来决定是否渲染视频。
> 2. 应用不会伪造车辆状态、不绕过任何行车安全限制，也绝不在车辆行驶过程中开启视频播放。
> 3. 当车辆状态变为限制行驶状态时，Mivu 会严格遵守 CarPlay 规范，立即暂停或调整视频输出。
> 4. Mivu 同时支持在 iPhone 端通过原生 AirPlay 投送视频。
>
> **主要使用场景：**
...
- **英文回复：**
  > Content is strictly user-provided through direct URL entry, local network UPnP/DLNA casting from the user's own devices on the same local network, or the user's personal media server. Mivu does not host, distribute, or scrape unauthorized proprietary streaming content.
- **中文含义：**
  > 内容完全由用户自行提供，包括手动输入 URL、同一局域网内用户自有设备的 UPnP/DLNA 投送，或用户自建的个人媒体服务器。Mivu 本身不托管、不上架、不抓取任何未经授权的第三方专有流媒体内容。

#### 问题 2：应用如何确保行车安全？（How does Mivu handle driving safety?）
- **英文回复：**
  > Mivu strictly adheres to `CPSessionConfiguration.supportsVideoPlayback`. Video rendering is only engaged when the CarPlay session explicitly reports that the vehicle allows video presentation. If the vehicle moves or disables video, Mivu suspends video output without any attempt to bypass system restrictions.
- **中文含义：**
  > Mivu 严格监听并服从 `CPSessionConfiguration.supportsVideoPlayback` 状态。只有车机系统明确允许显示视频时才会激活视频渲染；一旦车辆移动或车机切断视频允许状态，Mivu 会立即响应并停止视频渲染，绝不尝试任何绕过行为。

#### 问题 3：当前的分发与测试计划是什么？（What is the distribution plan?）
- **英文回复：**
  > In the current stage, Mivu is built for Development and Ad Hoc validation on registered private test devices and vehicles (e.g. validating CarPlay Video in Car capability on Infiniti Q50L in-car multimedia systems).
- **中文含义：**
  > 当前阶段，Mivu 主要用于已登记 UDID 的自有测试设备与车辆环境下的 Development / Ad Hoc 验证（例如验证英菲尼迪 Q50L 后装车机系统上的 CarPlay Video in Car 支持能力）。
...
2. **强调被动服从：**
   - 始终向 Apple 强调：**Mivu 是 100% 被动服从 CarPlay 车机协议状态的，车机允许才显示，车机禁止立刻停止**。
3. **定位清晰：**
